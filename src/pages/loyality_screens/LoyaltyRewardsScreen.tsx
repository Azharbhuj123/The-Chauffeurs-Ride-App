...
const rewards = [
    ...
    { id: '2', title: 'Upgrade Vehicle Class', cost: '500 Points', type: 'redeem' },
    { id: '3', title: 'Exclusive: Book Selected Driver', cost: 'Gold Tier Benefit', type: 'unlocked' },
];
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.balanceCard}>
                    ...
                    <Text style={styles.balanceSub}>Use your points for free rides & premium features.</Text>
                </View>
                        ...
                        <View key={item.id} style={styles.listItem}>
                            ...
                            {item.type === 'redeem' ? (
                                ...
                                <View style={[styles.redeemButton, styles.unlockedButton]}>
                                    <Text style={[styles.redeemButtonText, styles.unlockedButtonText]}>Unlocked</Text>
                                </View>
                            )}
                        </View>
                    ))}
                ...
                <View style={styles.listContainer2}>
                    {activities.map(item => (
                        <View key={item.id} style={styles.listItem}>
                            <Text style={styles.listItemTitle}>{item.title}</Text>
                            <Text style={[styles.activityPoints, item.points > 0 ? styles.pointsPositive : styles.pointsNegative]}>
            ...
            <Modal
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContainer}>
                        ...
                        <Ionicons name="star" size={wp(10)} color="#FDD835" style={{ marginBottom: hp('1%') }} />
                    ...
                    <View style={styles.btnContainer}>
                        <Button title='Confirm Redemption' onPress={() => setModalVisible(false)}/>
                    </View>
                    </View>
                </View>
            </Modal>
...
const styles = StyleSheet.create({
    ...
    sectionTitle: { fontSize: wp('4.5%'), fontWeight: '600', color: '#000', marginHorizontal: wp('5%'), marginTop: hp('3%'), marginBottom: hp('1.5%') },

    // Balance Card
    balanceCard: { backgroundColor: '#FDD835', borderRadius: 12, padding: wp('5%'), marginHorizontal: wp('5%'), alignItems: 'center', marginTop: hp(3) },
    balanceLabel: { fontSize: wp('3.8%'), color: '#333' },
    balancePoints: { fontSize: wp('8%'), fontWeight: 'bold', color: '#000', marginVertical: hp('0.5%'), marginTop: hp(1) },
    balanceSub: { fontSize: wp('3%'), color: '#333', marginTop: hp(1) },

    // Tiers Carousel
    tierCard: { width: wp('45%'), backgroundColor: '#fff', borderRadius: 12, padding: wp('4%'), marginRight: wp('4%'), borderWidth: 1, borderColor: '#eee', alignItems: 'center' },
    unlockedTierCard: { borderColor: '#FDD835' },
    tierName: { fontSize: wp('4%'), fontWeight: 'bold', color: '#000', marginTop: hp('1%') },
    tierPoints: { fontSize: wp('3.2%'), color: '#888', marginVertical: hp('0.5%') },
    tierPerks: { fontSize: wp('3.2%'), color: '#555', textAlign: 'center', marginTop: hp(1), paddingTop: hp(1), borderTopWidth: 1, borderTopColor: "#999" },

    // Lists (Redeem & Activity)
    listContainer: { marginHorizontal: wp('5%'), overflow: 'hidden' },
    listContainer2:{ marginHorizontal: wp('5%'), overflow: 'hidden',marginBottom:hp(10) },
    listItem: {
        flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: wp('4%'), backgroundColor: '#fff', marginBottom: 10, borderRadius: 12,


    },
    listItemTitle: { fontSize: wp('3.75%'), color: '#000', fontWeight: '500' },
    listItemSubtitle: { fontSize: wp('3.5%'), color: '#888', marginTop: hp('0.3%') ,color:"#FDD835" },
    redeemButton: { backgroundColor: '#FDD835', paddingVertical: hp('1%'), paddingHorizontal: wp('4%'), borderRadius: 20 },
    ...
    modalContainer: { width: wp('85%'), backgroundColor: '#fff', borderRadius: 20, padding: wp('6%'), alignItems: 'center' },
    closeButton: { position: 'absolute', top: wp('3%'), right: wp('3%') },
    modalTitle: { fontSize: wp('5.5%'), fontWeight: 'bold', color: '#000',marginTop: hp('2%') },
    modalSub: { fontSize: wp('3%'), color: '#666', marginTop: hp('1.75%') },
    modalItem: { fontSize: wp('5%'), fontWeight: '600', color: '#000', marginVertical: hp('0.5%') ,marginTop: hp('1.75%')},
    modalPointsBox: { width:'100%',backgroundColor: '#FFFBEA', borderColor: '#FFDDA9', borderWidth: 1, borderRadius: 12, padding: wp('3%'), marginVertical: hp('2%'), alignItems: 'center' },
    modalPointsLabel: { fontSize: wp('3.5%'), color: '#666' },
    modalPointsValue: { fontSize: wp('4.5%'), fontWeight: 'bold', color: '#000', marginTop: hp('0.5%') },
